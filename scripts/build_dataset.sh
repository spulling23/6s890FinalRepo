...
pgn-extract --quiet -ffilenames.txt --output combined.pgn
...
TAGS_FILE="tags.txt"
# Only create tags.txt if user did not provide one
if [ ! -f "$TAGS_FILE" ]; then
    echo 'TimeControl >= "300"' > "$TAGS_FILE"
fi
pgn-extract --quiet -t tags.txt combined.pgn --output time_control_gte_5m.pgn
rm -f combined.pgn 

# 5) Filter games that ended in checkmate
pgn-extract --quiet --checkmate time_control_gte_5m.pgn --output filtered_games.pgn
...
pgn-extract --quiet -#500000 shuffled_filtered_games.pgn
...
for p in [0-9]*.pgn; do
  base="${p%.pgn}"
  pgn-extract --quiet -Wfen  "$p" --notags --noresults                    --output "${base}.fens"
  pgn-extract --quiet -Wlalg "$p" --notags --nomovenumbers --nochecks --nocomments --nonags -w7 --output "${base}.moves"
done
